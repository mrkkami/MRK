...
class VPNConfigGUI:
    def __init__(self, root):
        ...
        self.shutdown_event = threading.Event() # NEW: Added shutdown event for graceful exit
    ...
    def setup_dark_theme(self):
        ...
        style.map('Stop.TButton',
                  background=[('!active', 'Tomato'), ('pressed', '3e3e3e')], # Corrected pressed state color
                  foreground=[('disabled', '#888888')])
    ...
    def show_speed_test_selection(self):
        ...
        self.test_url_combo.set("https://hero-wars.com")  # Default to "hero-wars.com"
    ...
    def process_logs(self):
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                # Check shutdown event IMMEDIATELY before trying to interact with GUI
                # If shutdown event is set, don't process further logs via GUI
                if self.shutdown_event.is_set(): # Corrected indentation for this block
                    break
                                    
                if self.root.winfo_exists(): # Corrected indentation for this block
                    self.root.after(0, self.update_terminal, message)
                else: # Corrected indentation for this block
                    # If window is gone, just print to console (should be caught by shutdown event now)
                    print(f"GUI closed, skipping log: {message}")
            except queue.Empty:
                continue
            except Exception as e:
                # This print will still occur if an error happens *before* shutdown event is effective
                print(f"Error in process_logs: {e}")
        # print("Log processing thread stopped cleanly.") # Optional: for debug confirmation
            ...
            for filename in os.listdir(self.TEMP_FOLDER):
                ...
                try:
                    if os.path.isfile(file_path):
                        os.unlink(file_path)
                    elif os.path.isdir(file_path): # Ensure directories are also handled
...
def main():
    ...
    # NEW: Handle window close event for graceful shutdown
    def on_closing():
        if messagebox.askokcancel("Quit", "Do you want to quit?"):
            app.log("Application is closing down...")
            app.shutdown_event.set() # Set the shutdown event to stop background threads
            # Give background threads a moment to react to the shutdown event
            time.sleep(0.5) 
            app.kill_existing_xray_processes_instance() # Ensure Xray is killed
            app.clear_temp_folder() # Clean up temp files
            root.destroy() # Destroy the Tkinter window

    root.protocol("WM_DELETE_WINDOW", on_closing) # Bind the custom closing function
